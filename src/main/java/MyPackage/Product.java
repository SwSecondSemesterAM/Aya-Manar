...
public class Product {
	...
	public static void createProduct(int nom, String cId, String dimention, String material, String color,
			String category, String payType, int quantity, String pic, double price, int rate) {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(PRODUCT_FILENAME))) {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (data[1].equals(customerId)) { // check if the customer ID matches the desired ID
					System.out.println("order with number " + data[0] + "  category " + data[2] + " Material " + data[3]
					...
					if (data[9].equalsIgnoreCase("complete")) {
						...
						String updatedLine = String.join("\t", data); // join the array values back into a tab-separated
																		// string
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (id == idToDelete && status.equals("waiting")) {
					LOGGER.log(java.util.logging.Level.SEVERE,
							PRODUCT_NUMBER_PREFIX + idToDelete + " and status waiting deleted!");
				} else if (id != idToDelete) {
					writer.write(line + System.lineSeparator());
				} else {
					LOGGER.log(java.util.logging.Level.WARNING,
							PRODUCT_NUMBER_PREFIX + idToDelete + " cannot be deleted due to invalid status");
				}
	...
	public static void updateProduct(int orderNumToUpdate, String cusId, String category, String material, String color,
			String dimention, String payType, int quantity, String picture, String status, String isReq, String price,
			int rate) {
		...
		try {
			...
			while ((line = reader.readLine()) != null) {
				...
				if (id != orderNumToUpdate) {
					...
					String updatedLine = orderNumToUpdate + "\t" + cusId + "\t" + category + "\t" + material + "\t"
							+ color + "\t" + dimention + "\t" + payType + "\t" + quantity + "\t" + picture + "\t"
							+ status + "\t" + isReq + "\t" + price + "\t" + rate;
